...
import { Button, FormGroup, InputGroup, Form, Input, Label } from "reactstrap";
...
const Home = () => {
  ...
  const [latestPosts, setLatestPosts] = useState([
    {
      _id: 1,
      video:
        "https://ritzplumbing.com/wp-content/uploads/2021/05/palm-trees.mp4",
  ...
  const getAllSlides = async () => {
    try {
      ...
      const response = await getRequest(`/api/secure/site/header-list`, token);
  ...
  const getPageVideo = async () => {
    try {
      const token = localStorage.getItem("TOKEN");
      const response = await getRequest(`/api/secure/site/`, token);
  ...
  const getLatestPosts = async () => {
    try {
      const token = localStorage.getItem("TOKEN");
      const response = await getRequest(`/api/secure/post/post-list`, token);

      setLatestPosts(response.result.data.posts);

      console.log("latestPosts", response.result.data.posts);
    } catch (error) {
      console.log("Get Site Setting Error", error);
    }
  };
  const getEventUpdates = async () => {
    try {
      const token = localStorage.getItem("TOKEN");
      const response = await getRequest(`/api/secure/event/event-list`, token);
  ...
  const getParties = async () => {
    try {
      const token = localStorage.getItem("TOKEN");
      const response = await getRequest(`/api/secure/party/party-list`, token);
  ...
  const signUpHandler = async (e) => {
    ...
    formData.append("middleName", e.target[1].value);
    formData.append("lastName", e.target[2].value);
    formData.append("email", e.target[3].value);
    formData.append("phone", e.target[4].value);
    if (changeAge) {
      formData.append("age", changeAge.value);
    }
    formData.append("address", e.target[6].value);
    formData.append("fbLink", e.target[7].value);
    if (changeProfession) {
      formData.append("profession", changeProfession.value);
    }
    if (changeParty) {
      formData.append("partyId", changeParty.value);
    }
    formData.append("password", e.target[10].value);
    formData.append("confirm_password", e.target[11].value);
    const APIresponse = {
    ...
    // }
    try {
      const response = await postRequest("/api/pub/auth/register", APIresponse);
              ...
              {slides.map((e, index) => {
                ...
                return (
                  <Carousel.Item key={`id_${e._id}_${index}`}>
                    <Image
                      src={
                        "https://sambayan-1.s3.ap-south-1.amazonaws.com/" +
                        e.backgroundImage
                      }
                            ...
                            <Row className="justify-content-center">
                              <Col lg={10} md={10} xs={12}>
                                <Image
                                  ...
                                  fluid
                                />
                                {/* <a classuName="btn btn-default mt-4" href={e.buttonURL}>Click to know more</a> */}
                              </Col>
                            </Row>
                          </Container>
                        </Carousel.Caption>
                ...
                <Col lg={10} md={10} xs={12}>
                  <h1>{latestPostsSection.heading}</h1>
                  {
                    latestPosts.length > 4
                      ? _.sortBy(
                          latestPosts.slice(
                            Math.max(latestPosts.length - 5, 1)
                          ),
                          "total"
                        )
                          .reverse()
                          .map((e, index) => (
                            <PostCard
                              readMore={true}
                              link={`/singlePost?_id=${e._id}`}
                              key={`id_${e._id}_${index}`}
                              heading={e.title}
                              text={e.description}
                              time={e.createdAt}
                              video={
                          ...
                          >
                            ...
                            <Row className="mt-5">
                              ...
                              <Col xs={6}>
                                <Link
                                  to="/login"
                                  className="btn-md btn link text-white w-100"
                                >
                                  Sign In
                                </Link>
                              </Col>
