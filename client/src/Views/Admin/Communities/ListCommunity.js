...
const ExpandedComponent = ({ data }) => {
    const [defaultData, setDefaultSlideData] = useState({
        ...
        image: "https://1sambayan-env.eba-5wmwf5mk.us-east-1.elasticbeanstalk.com/views/uploads/" + data.image,
        backgroundImage: "https://1sambayan-env.eba-5wmwf5mk.us-east-1.elasticbeanstalk.com/views/uploads/" + data.backgroundImage,
    })
    ...
    const getAllPosts = async () => {
        try {
            ...
            // response.result.data.posts.map((item, key) => {
                ...
                setTableData(response.result.data.communities);
            // });
            console.log("Get All Posts Response", response.result.data.communities);
        } catch (error) {
    ...
    const deleteSinglePost = async (postId, row, index, column, id) => {
        try {
            ...
            const response = await postRequestForm(
                `/api/secure/community/delete-community`,
                token, {postId}
            );
