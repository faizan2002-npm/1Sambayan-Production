...
const ExpandedComponent = ({ data }) => {
    ...
    const updateSlideHandler = async (e) => {
        ...
        const APIresponse = {
            ...
            desigination: formData.get('desigination'),
        ...
        try {
            ...
            if (response.result.status === 200) {
                toast.success('Convenor Updated', {
        ...
        <Form className="form" encType="multipart/form-data" method="post" onSubmit={updateSlideHandler}>
            <Row className="m-0">
                <Col xs={12} className="my-3">
                    <h2>Edit Convenor {data.title}</h2>
                </Col>
                <Col xs={12}>
...
const ListConvenors = () => {
    ...
    const getAllPosts = async () => {
        try {
            ...
            setTableData(response.result.data.convenors);
    ...
    const deleteSinglePost = async (postId, row, index, column, id) => {
        try {
            ...
            const response = await postRequestForm(
                `/api/secure/convenor/delete-convenor`,
                token, { postId }
            );
            // console.log(response);
            if (response.result.status === 200) {
                toast.success('Convenor Deleted', {
                    position: "bottom-right",
                    autoClose: 2000,
                    hideProgressBar: false,
                    closeOnClick: true,
